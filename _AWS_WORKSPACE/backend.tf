terraform {
  backend "s3" {
    ...
    key            = "s3-project/${terraform.workspace}/terraform.tfstate"
    region         = "us-east-1"
...
# ${terraform.workspace} injects the workspace name (e.g., dev, prod) into the state file path, 
# so dev state is stored in s3-project/dev/terraform.tfstate and 
#prod in s3-project/prod/terraform.tfstate.
