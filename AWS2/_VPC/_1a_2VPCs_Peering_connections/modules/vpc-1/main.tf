...
resource "aws_subnet" "public_subnet" {
  ...
  tags = {
    Name = "${var.environment}-vpc1-public-subnet"
  }
}
...
resource "aws_security_group" "public_sg" {
  vpc_id                  = aws_vpc.main.id
   ...
   tags = {
     Name        = "${var.environment}-vpc1-public-sg"
   }
}
...
resource "aws_instance" "public_instance" {
   ...
   tags = {
      Name        ="${var.environment}-VPC-1-public-instance"
   }
}
...
resource "aws_subnet" "private_subnet" { 
  vpc_id                  = aws_vpc.main.id
   ...
   tags              ={ 
      Name          ="${var.environment}-vpc1-private-subnet"
   }
}
...
ingress {
   ...
   tags ={ 
      Name          ="${var.environment}-vpc1-private-sg"
   }
}
...
resource "aws_instance" "private_instance" { 
  ...
  key_name      = var.key_name
   ...
   # user_data = file("install.sh") 
   tags ={ 
         Name          ="${var.environment}-VPC-1-private-instance"
   }
}
