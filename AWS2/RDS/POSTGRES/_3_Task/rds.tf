...
# this block will generate a random string that we can use as a suffix for our RDS snapshot name, to ensure uniqueness. This is especially useful if you have multiple environments (dev, staging, prod) and want to avoid naming conflicts when creating snapshots on destroy.
