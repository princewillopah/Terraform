...
# Private RTs (no internet route) – peering routes added later

resource "aws_route_table" "a_private_rt" {
  provider = aws.use1
  vpc_id   = aws_vpc.vpc_a.id
}

resource "aws_route_table" "b_private_rt" {
  provider = aws.usw2
  vpc_id   = aws_vpc.vpc_b.id
}

resource "aws_route_table" "c_private_rt" {
  provider = aws.euw1
  vpc_id   = aws_vpc.vpc_c.id
}
