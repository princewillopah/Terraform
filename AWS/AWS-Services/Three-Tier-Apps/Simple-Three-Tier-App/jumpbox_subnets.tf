resource "aws_subnet" "jumpbox_subnet_1c" {
  vpc_id                  = aws_vpc.vpc.id
  cidr_block              = "10.10.9.0/24"
  availability_zone       = "us-east-1a"
  map_public_ip_on_launch = true

  tags = { Name = "${var.app_name}-jumpbox-sub-1a" }
}
...
resource "aws_route_table_association" "jumpbox_subnet_1a_association" {
  subnet_id      = aws_subnet.jumpbox_subnet_1c.id
  route_table_id = aws_route_table.jumpbox_RT.id
}
