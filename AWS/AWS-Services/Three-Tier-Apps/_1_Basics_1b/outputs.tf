...
output "ssh_key_to_jump_server" {
  value = "scp -i ~/DevOps/ssh-keys/Princewill-ssh-bayero-sub.pem ~/DevOps/ssh-keys/Princewill-ssh-bayero-sub.pem ubuntu@${aws_instance.app_Server_1a.public_ip}:/home/ubuntu/"
}
...
output "app_server_1a_connectivity_string" {
  value = "ssh -i ~/DevOps/ssh-keys/Princewill-ssh-bayero-sub.pem ubuntu@${aws_instance.app_Server_1a.public_ip}"
}
